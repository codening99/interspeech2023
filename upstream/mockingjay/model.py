...
class TransformerSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        ...
        # attention_scores: (batch_size, head_num,l seqlen, seqlen)
...
class TransformerSpecMAMPredictionHead(nn.Module):
    """
    MAM预测头
    """

    def __init__(self, config, output_dim, input_dim=None):
        super(TransformerSpecMAMPredictionHead, self).__init__()
...
class TransformerSpecScorePredictionHead(nn.Module):
    """
    情感强度预测头
    """

    def __init__(self, config, output_dim, input_dim=None):
        super(TransformerSpecScorePredictionHead, self).__init__()
        self.output_dim = output_dim
        if input_dim is None:
            self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        else:
...
class TransformerSpecAWEPredictionHead(nn.Module):
    """
    声学词向量预测头
    """

    def __init__(self, config, output_dim, input_dim=None):
        super(TransformerSpecAWEPredictionHead, self).__init__()
        self.output_dim = output_dim
        if input_dim is None:
            self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        else:
