...
def get_preprocess_args():
    ...
    parser.add_argument('-i', '--input_data', default='../ESDcorpus/', type=str,
                        help='Path to your ESDcorpus directory', required=False)
    parser.add_argument('-o', '--output_path', default='./data/', type=str, help='Path to store output', required=False)
    parser.add_argument('-a', '--audio_extension', default='.wav', type=str,
                        help='audio file type (.wav / .flac / .mp3 / etc)', required=False)
    parser.add_argument('-n', '--name', default='len_for_bucket', type=str, help='Name of the output directory',
                        required=False)
    parser.add_argument('--n_jobs', default=-1, type=int, help='Number of jobs used for feature extraction',
                        required=False)
...
def generate_length(args, tr_set, audio_extension):
    for i, s in enumerate(tr_set):
        ...
        print('')
        todo = sorted(list(Path(os.path.join(args.input_data, s)).rglob('*' + audio_extension)))  # '*.wav'
        ...
        # 排序
        indices = [idx for idx in reversed(np.argsort(tr_x))]

        # 将分数插入到length csv文件中
        # file_path = "./data/len_for_bucket"
        # score_df = pd.read_csv(os.path.join(file_path, "score_list.csv"))
        # 根据len对score进行排列
        # sorted_score = [score_df.loc[idx, 'score'] for idx in indices]

        # sort by len
        sorted_todo = [os.path.join(s, str(todo[idx]).split(s + '/')[-1]) for idx in indices]
