...
from upstream.mockingjay.model import TransformerSpecMAMPredictionHead, TransformerModel, TransformerSpecScorePredictionHead
...
class UpstreamPretrainExpert(nn.Module):
    ...
    def forward(self, data, records={}, global_step=0, log_step=1000, **kwargs):
        ...
        Args:
            data:
                [[spec_masked, spec_score], pos_enc, mask_label, attn_mask, [spec_target, spec_score_target]]
        ...
        spec_masked_score = spec_masked[1]
        ...
        if global_step % log_step == 0:
            spec_list = [spec_masked, pred_spec_mam, spec_target]
            name_list = ['mask_spec', 'pred_spec_mam', 'true_spec']
...
class TransformerForMaskedAcousticModel(TransformerInitModel):
    ...
    def __init__(self, config, input_dim, output_dim, output_attentions=False, keep_multihead_output=False):
        ...
        # 情感分数预测头
        self.SpecScoreHead = TransformerSpecScorePredictionHead(config, output_dim if output_dim is not None else input_dim)
    ...
    def forward(self, spec_input, pos_enc, mask_label=None, attention_mask=None, spec_label=None, head_mask=None):
        ...
        # MAM输入
        spce_masked = spec_input[0]
        # MAM ground_true
        spec_label_mam = spec_label[0]

        # 情感分数输入
        spec_score = spec_input[1]
        # 情感分数 ground_true
        spec_label_score = spec_label[1]
        ...
        # MAM输出
        outputs_mam = self.transformer_mam

        # 情感分数输出
        outputs_score = self.transformer_score
        ...
        if spec_label is not None and mask_label is not None:
            ...
            # MAM损失函数
            masked_spec_loss = self.loss(pred_spec_mam.masked_select(mask_label), spec_label_mam.masked_select(mask_label))

            # 情感分数损失函数
            score_spec_loss = self.loss(pred_spec_score.masked_select(mask_label), spec_label_score.masked_select(mask_label))

            # print("损失函数\n")
            # print(masked_spec_loss)
            # print("\n")
