...
class Runner():
    ...
    def _get_upstream(self):
        ...
        # 上游模型路径
        # self.args.upstream: mockingjay
        module_path = f'pretrain.{self.args.upstream}.pretrain_expert'

        # 获取上游模型
        # Upstream: pretrain/mockingjay/pretrain_expert/UpstreamPretrainExpert
            ...
            for data in tqdm(dataloader, dynamic_ncols=True, desc='train'):
                ...
                try:
                    ...
                    with torch.cuda.amp.autocast(enabled=amp):
                        ...
                        # self.upstream: pretrain/mockingjay/UpstreamPretrainExpert.forward
                    ...
                    if gradient_accumulate_steps > 1:
                        loss_mam = loss_mam / gradient_accumulate_steps
                        loss_score = loss_score / gradient_accumulate_steps
                    if self.args.multi_gpu:
                        loss_mam = loss_mam.sum()
                        loss_score = loss_mam.sum()
                    if amp:
                        scaler.scale(loss_mam).backward()
                ...
                # record loss
                all_loss = all_loss + loss_mam.item() + loss_score.item()

                mam_loss.append(loss_mam.item())
                losses = torch.tensor(mam_loss)

                for n_iter in range(len(losses)):
                    writer.add_scalar(tag='Loss/mam_score',
                    scalar_value=losses[n_iter],
                    global_step=n_iter)
                del loss
                ...
                # gradient clipping
                grad_norm = torch.nn.utils.clip_grad_norm_(self.upstream.model.parameters(),
